...
// PUT /api/v1/ekaer-records/:id/edit - Szerkesztett HTML visszamentése DOCX-ként
router.put('/:id/edit', async (req, res) => {
  try {
    ...
    if (!record || !record.file_path) {
      return res.status(404).json({ status: 'error', message: 'EKAER rekord vagy fájl elérési út nem található.' });
    }
    ...
    // 1. Biztonsági mentés egy külön mappába
    const backupDir = path.join(process.env.RAKTAR_PATH || '/mnt/raktar', '_DOCX_Backup', 'EKAER');
    ...
    res.json({ status: 'success', message: 'EKAER dokumentum sikeresen mentve.' });
  } catch (error) {
    console.error('Hiba az EKAER dokumentum szerkesztésekor:', error);
