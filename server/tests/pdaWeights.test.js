...
const end = source.indexOf("router.put('/commission-lines/:id/pick'", start);
...
function fixture() {
  ...
  const trx = table => ({
    ...
    async insert(row) { logs.push(row); },
...
test('8 then 7 cartons uses two then one pallets and accumulates net weight', async () => {
  const f = fixture();
  await processPick(f.trx, 1, data(8, 100, 8));
  assert.equal(f.line.net_weight, 48);
  await processPick(f.trx, 1, data(7, 100, 7));
  assert.equal(f.line.gross_weight, 200);
  assert.equal(f.line.net_weight, 119);
  assert.deepEqual(f.logs.map(row => row.pallets), [2, 1]);
  assert.ok(f.logs.every(row => row.aldi_truck_id && row.product_name));
});
...
test('zero net is saved and a different item starts its own pallets', async () => {
  ...
  await processPick(a.trx, 1, data(8, 52, 8));
  await processPick(b.trx, 2, data(7, 100, 7));
  assert.equal(a.line.net_weight, 0);
