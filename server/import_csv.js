  ...
  try {
    ...
    async function getSeasonId(code) {
      ...
      const [newId] = await db('seasons').insert({
        code: code,
        start_date: `20${code.split('-')[0]}-05-01`,
        end_date: `20${code.split('-')[1]}-04-30`
      }).returning('id');
    ...
    async function getTransporterId(name) {
      ...
      const [newId] = await db('transporters').insert({ name: name.trim(), code: key.substring(0,3) }).returning('id');
