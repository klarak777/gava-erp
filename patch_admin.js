...
adminContent = adminContent.replace(
    "${extraPayload.isReadonly ? '' : '<th>Műveletek</th>'}",
    "${(extraPayload.isReadonly && !extraPayload.allowReassign) ? '' : '<th>Műveletek</th>'}"
);
...
// 5b. Update the archive reassign logic
archiveMod = archiveMod.replace(
    "const isIdenActive = !iden.is_inactive;",
    "const isIdenActive = !iden.is_inactive;" // Wait, it's not there.
);
archiveMod = archiveMod.replace(
    "<td>\n                                <button class=\"arch-btn activate btn-activate-iden\"",
    "<td>\n                                ${!iden.is_inactive ? '' : '<button class=\"arch-btn activate btn-activate-iden\" data-id=\"' + iden.id + '\">Aktiválás</button>'}"
);
