...
// 1. Replace header Műveletek check
c = c.replace(
...
// 2. Replace row Td check using Regex
const rowTdRegex = /\$\{extraPayload\.isReadonly \? '' : `[\s\S]*?<\/td>`\}/;
...
c = c.replace(rowTdRegex, newRowTd);

// 3. Replace event handler block
const eventRegex = /if \(!extraPayload\.isReadonly\) \{\s*tbody\.querySelectorAll\('\.edit-btn'\)[\s\S]*?\}\s*\}/;
...
c = c.replace(eventRegex, newEvents);

// 4. Append openArchivedPartnersTable if missing!
if (!c.includes('export function openArchivedPartnersTable')) {
    const archiveCode = `
...
console.log('Successfully patched admin.js using regex!');
