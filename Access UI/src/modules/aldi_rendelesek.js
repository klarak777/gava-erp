...
export function renderAldiRendelesek(container, windowManager) {
  ...
  function renderHetiLekotesHtml() {
    ...
    const days = [
      { name: 'Szerda', key: 'wed' },
      { name: 'Csütörtök', key: 'thu' },
      { name: 'Péntek', key: 'fri' },
      { name: 'Szombat', key: 'sat' },
      { name: 'Vasárnap', key: 'sun' },
      { name: 'Hétfő', key: 'mon' },
      { name: 'Kedd', key: 'tue' }
    ];
    ...
    return `
      <div style="margin-top:16px;">
        <div style="background:#f8fafc; border:1px solid #e2e8f0; border-radius:12px; padding:16px; margin-bottom:24px; display:flex; justify-content:space-between; align-items:center; flex-wrap:wrap; gap:16px;">
          <div style="display:flex; align-items:center; gap:16px;">
            <div style="display:flex; align-items:center; gap:8px;">
              <label style="font-size:13px; font-weight:600; color:#475569;">Év:</label>
              <select id="aldi-lekotes-year-select" style="padding:6px 12px; border-radius:6px; border:1px solid #cbd5e1; font-size:13px; outline:none; cursor:pointer;">
                ${buildYearOptions(state.hetiLekotesYear)}
              </select>
            </div>
            <div style="display:flex; align-items:center; gap:8px;">
        ...
        ${!state.hetiLekotesSelectedWeek ? `
        <div style="background:#ffffff; border:1px solid #e2e8f0; border-radius:12px; padding:40px; text-align:center;">
          <p style="margin:0; font-size:13px; color:#64748b;">Kérlek válassz évet és hetet az adatok megtekintéséhez.</p>
        </div>
        ` : `
        <div style="background:#ffffff; border:1px solid #e2e8f0; border-radius:12px; overflow:auto; margin-bottom:24px;">
          <h4 style="margin:16px; font-size:15px; color:#1e293b;">Lekötés & Rendelés</h4>
          <table style="width:100%; border-collapse:collapse; font-size:13px; text-align:left;">
            <thead>
              <tr style="background:#f1f5f9; border-bottom:2px solid #cbd5e1;">
                <th rowspan="2" style="padding:8px; border:1px solid #e2e8f0;">Termék</th>
                <th rowspan="2" style="padding:8px; border:1px solid #e2e8f0; text-align:center;">Akció időszak</th>
                <th rowspan="2" style="padding:8px; border:1px solid #e2e8f0; text-align:center;">KW${state.hetiLekotesSelectedWeek}</th>
                ...
                <th colspan="2" style="padding:8px; border:1px solid #e2e8f0; text-align:center;">Szombat</th>
              ...
              <tr style="background:#f8fafc; border-bottom:1px solid #e2e8f0;">
                ${days.map(d => `<th style="padding:6px; border:1px solid #e2e8f0; text-align:center; font-size:11px;">Rendelt</th><th style="padding:6px; border:1px solid #e2e8f0; text-align:center; font-size:11px; color:#64748b;">Becsült</th>`).join('')}
              </tr>
            </thead>
            <tbody>
              ${lekotesRows}
        ...
        <div style="background:#ffffff; border:1px solid #e2e8f0; border-radius:12px; overflow:auto;">
          <h4 style="margin:16px; font-size:15px; color:#1e293b;">Készlet lista</h4>
          <table style="width:100%; border-collapse:collapse; font-size:13px; text-align:left;">
            <thead>
              <tr style="background:#f1f5f9; border-bottom:2px solid #cbd5e1;">
                <th rowspan="2" style="padding:8px; border:1px solid #e2e8f0;">Termék</th>
      ...
      <div style="display:grid; grid-template-columns: 1fr 1fr 1fr; gap:20px; margin-top:20px; max-width:1200px;">
        ...
        state.editingBlock === 'carton' ? `
               ...
               <div style="border:1px dashed #cbd5e1; padding:16px; background:#f8fafc; text-align:center; font-size:12px; line-height:2.0; display:flex; flex-direction:column; gap:6px;">
                 <div><input type="text" id="aldi-inline-product-name" value="${customTexts.product_name || selectedProd.name || 'TERMÉKNÉV'}" class="access-control-input" style="font-size:14px; font-weight:800; text-transform:uppercase; text-align:center; width:100%; border:1px solid #cbd5e1; padding:2px; height:24px;"></div>
                 <div style="display:flex; justify-content:center; align-items:center; gap:4px;">
                   <input type="text" id="aldi-inline-class" value="${selectedProd.label_class || ''}" class="access-control-input" style="width:40px; text-align:center; padding:2px; height:24px;">
                   <input type="text" id="aldi-inline-class-lbl" value="${customTexts.lbl_class || 'oszt.'}" class="access-control-input" style="width:50px; text-align:center; padding:2px; height:24px; border:none; background:#e2e8f0; border-radius:4px;">
                   <input type="text" id="aldi-inline-size-lbl" value="${customTexts.lbl_size || 'Méret:'}" class="access-control-input" style="width:55px; text-align:center; padding:2px; height:24px; border:none; background:#e2e8f0; border-radius:4px;">
                 ...
                 <div style="display:flex; justify-content:center; align-items:center; gap:6px;">
                   <input type="text" id="aldi-inline-origin-lbl" value="${customTexts.lbl_origin || 'Származási hely:'}" class="access-control-input" style="width:120px; text-align:right; padding:2px; height:24px; border:none; background:#e2e8f0; border-radius:4px;">
                   <input type="text" id="aldi-inline-origin" value="${selectedProd.label_origin || ''}" class="access-control-input" style="width:140px; padding:2px; height:24px;" placeholder="Magyarország">
                 </div>
                 ...
                 <div style="display:flex; justify-content:center; align-items:center; gap:4px; font-weight:700;">
                   <input type="text" id="aldi-inline-weight-lbl" value="${customTexts.lbl_weight || 'Nettó tömeg:'}" class="access-control-input" style="width:90px; text-align:right; padding:2px; height:24px; border:none; background:#e2e8f0; border-radius:4px; font-weight:bold;">
                   <input type="text" id="aldi-inline-weight-c" value="${selectedProd.label_net_weight_carton || ''}" class="access-control-input" style="width:80px; text-align:center; padding:2px; height:24px;">
                 </div>
               </div>
             ...
             ` : `
               ...
               <div style="border:1px solid #cbd5e1; padding:16px; background:#f8fafc; text-align:center; font-size:12px; line-height:1.6; display:flex; flex-direction:column; gap:4px;">
                 ...
                 <div style="font-weight:700;">${customTexts.lbl_weight || 'Nettó tömeg:'} ${selectedProd.label_net_weight_carton || '-'}</div>
               </div>
               <div style="display:flex; gap:10px; margin-top:12px; justify-content:center;">
  ...
  function bindEvents() {
    ...
    // Heti lekötés vezérlők
    const hetiLekotesYearSelect = wrapper.querySelector('#aldi-lekotes-year-select');
    if (hetiLekotesYearSelect) {
    ...
    const hetiLekotesWeekSelect = wrapper.querySelector('#aldi-lekotes-week-select');
    if (hetiLekotesWeekSelect) {
      hetiLekotesWeekSelect.addEventListener('change', (e) => {
        state.hetiLekotesSelectedWeek = e.target.value ? parseInt(e.target.value, 10) : null;
        fetchHetiLekotesData();
      });
    }
